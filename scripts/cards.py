...
projects.json when set, otherwise the repo's own GitHub description. A projects.json
entry may be "owner/repo" to feature a repo you contribute to but do not own.
...
def main(argv=None):
    ...
    for entry in wanted:
        # "owner/repo" fetches directly, so a project you only collaborate on
        # (rather than own) still gets a card with live star/fork counts.
        if "/" in entry["repo"]:
            try:
                src = rest(f"/repos/{entry['repo']}", token)
            except Exception as exc:
                print(f"  !! {entry['repo']} could not be fetched ({exc}), skipped")
                continue
        else:
        ...
        if not src:
            ...
            continue
        # "name" in projects.json renames the card, for when the repo slug
        # isn't what the project is actually called (Medtech-Wellora -> Wellora).
        display = entry.get("name") or src["name"]
        card = {
            "name": display,
        ...
        for theme in ("dark", "light"):
            dest = args.out / f"card-{display}-{theme}.svg"
            dest.write_text(render_repo(card, theme), encoding="utf-8")
        print(f"wrote card-{display}-*.svg  "
